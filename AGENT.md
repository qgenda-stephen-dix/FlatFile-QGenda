...
1. **Custom Validations**: Avoid using `@flatfile/plugin-constraints` for custom field-level validations. Use Record Hooks instead.
...
4. **Blueprint**: Avoid using `external` constraint type to reference custom validations. Use Record Hooks instead.
...
bun install @flatfile/plugin-record-hook
```
